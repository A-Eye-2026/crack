...
app.config['UPLOAD_BASE_DIR'] = UPLOAD_BASE_DIR
app.config['UPLOAD_IMAGE_DIR'] = UPLOAD_IMAGE_DIR
app.config['UPLOAD_VIDEO_DIR'] = UPLOAD_VIDEO_DIR
app.config['DEFAULT_RADIUS_M'] = 500
...
def inject_global_vars():
    ...
    if session.get('user_id') and user_role in ('admin', 'manager'):
        row = db.session.execute(
            text("""
                SELECT COUNT(*) AS cnt
                FROM report
                WHERE status = '관리자 확인중'
            """)
        ).mappings().first()

        admin_unread_count = int(row.get('cnt', 0)) if row else 0
...
def index():
    ...
    if row:
        resolved_role = row.get('role_value') or ('admin' if row.get('is_admin') else 'user')

        session['is_admin'] = (resolved_role == 'admin') or bool(row.get('is_admin'))
